...
def main():
    ...
    owner = config['repositories'][0]['owner'] # Get the repository owner from the config dictionary
    repo = config['repositories'][0]['repo'] # Get the repository name from the config dictionary
    output_path = 'issues.json' # Path to the JSON file where the issues will be saved
