...
def main():
    ...
    for repo_config in config['repositories']: # Loop through each repository configuration in the 'repositories' list from the config dictionary
        ...
        # Add the fetched issues to the array using the append() method (adds the entire list of issues as a single element in the 'all_issues' list/collection)
    ...
    print(f"Saved {len(all_issues)} issues to {output_path}") # Print a confirmation message that all issues have been saved to the specified JSON file
